...
import { Share, MessageSquare, Crown } from 'lucide-react';
...
export const VideoDetail: React.FC = () => {
  ...
  const isTopHype = sortedVideos[0]?.id === currentVideo.id;
  const related = sortedVideos.filter(v => v.id !== currentVideo.id).slice(0, 15);
      ...
      <div className="st-player-wing">
        ...
        <div style={{ padding: '30px 40px', backgroundColor: 'var(--bg-darker)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              {/* Top Row: Relative Time with Accent Bar */}
            ...
            <div style={{ display: 'flex', gap: '10px', marginLeft: '20px' }}>
              <button className="st-btn" style={{ backgroundColor: 'var(--bg-light)', color: 'var(--text-main)', border: 'none', padding: '8px 16px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700, fontSize: '13px' }}>
                <Share size={16} /> Compartir
              </button>
            </div>
          </div>

          {/* User Info (Twitch style below stats) */}
